...
const VERSIONS = [
  {
    version: "Ver.1-260913-1705",
    date: "2026-09-13",
    changes: [
      "プレイオフ日程（53rd_schedule_playoff.htm）を取得対象に追加",
      "試合番号が連番でない行（プレイオフの SF1・空欄など）も試合として取り込むよう修正",
      "シーズン判定を10月開幕基準に修正し、53rd/54th を自動で切り替えるよう変更",
      "プレイオフ表の列構造（14列）に対応し、回戦名（Quarter Finals 等）をバッジ表示",
      "スコアを日付の新しい順に表示（従来は古い順）",
      "スコアの取得元も53rd固定をやめ、シーズンを自動判定（54th開幕後も継続表示）",
  ...
  },
  {
    ...
    date: "2026-09-11",
    changes: [
      "スケジュール・レンタルの取得元URLを日付から自動生成（公式サイトに新しい月が追加されても自動で反映）",
      "取得できていない場合に警告バナーを表示（「試合なし」と区別できるように）",
      "cronを実監視に変更: 公式サイトを直接確認し、0件・取得失敗を検知したらエラーを返す",
      "cron実行時にAPIキャッシュを破棄・再生成するよう修正（従来のキャッシュ回避は効いていなかった）",
      "順位表の取得キャッシュがルートのキャッシュより長く、最大5日古くなる問題を修正",
      "月フィルタが年をまたいでも重複しないよう表記を改善",
    ],
  },
  {
    version: "Ver.1-260405-2255",
    date: "2026-04-05",
    changes: [
      "APIキャッシュ最適化（スケジュール1h・順位表48h・前シーズン1日）で表示速度を大幅改善",
      "ページ読み込み中のスケルトンUI追加（真っ黒画面の解消）",
      "イベントページ: イベント・プログラム紹介記事をタップで詳細モーダル表示",
      "リンク予定ページ: イベントプログラムと自動マッチングし「詳細」バッジ＋モーダル表示",
      "順位表デバッグ機能を /api/standings-debug に分離",
      "ハンバーガーメニューから個人ランキング検索リンクを削除",
    ],
  },
  {
