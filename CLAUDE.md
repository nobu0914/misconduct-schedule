...
`Ver.1-260924-0222`（Nav.tsx の h1 タグ内に表示）
...
### 2026-09-24（表示期間）
- 「今後のみ表示」が初期ONで、**終わった直後の試合が即座に見えなくなる**ため、9/19-20 のプレイオフが
  TOPから消えていた。過去分も残すよう変更（`src/app/page.tsx` の `isUpcoming()`）。
  - 通常の試合: 過去7日（`RECENT_DAYS`）
  - プレイオフ（`round` が付く試合）: 過去30日（`RECENT_PLAYOFF_DAYS`）。シーズンの締めくくりで本数も少ないため
  - フィルタのラベルも実態に合わせて「今後のみ」→「直近・今後」に変更
  - レンタル（`/rental`）は過去の枠を見ても意味がないので従来どおり当日以降のみ
