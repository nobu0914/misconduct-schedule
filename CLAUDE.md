...
`Ver.1-260913-1654`（Nav.tsx の h1 タグ内に表示）
...
- ただし「0件＝異常」ではない。枠だけ先に公開された未記入ページ（`54th_schedule_march.htm` など）で誤検知したため、
  **日付行が1件以上あるのに試合行が0件**のときだけ構造変更を疑うようにした。レンタルは公開直後に予定0件が普通にあるので0件では警告しない。
